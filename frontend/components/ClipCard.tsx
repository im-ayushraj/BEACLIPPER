...
import { Download, Trash2, Loader2, Clock, Maximize2, Scissors } from "lucide-react";
import { Clip } from "@/types";
import { formatTime } from "@/lib/utils";
import { VideoPlayer } from "./VideoPlayer";
import { ClipTheaterModal } from "./ClipTheaterModal";
        ...
        <div className="p-4 flex flex-col gap-3 flex-1">
          ...
          <div className="mt-auto pt-3 border-t border-white/[0.06] flex items-center gap-2">
            ...
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1.5 rounded-lg bg-white px-3 py-2 text-xs font-semibold text-black transition hover:bg-zinc-200 active:scale-[0.98] disabled:opacity-75 cursor-pointer"
            >
              {isDownloading ? (
                <>
            ...
            {/* Preview & Trim / Theater Mode Button */}
            <button
              type="button"
              onClick={() => setTheaterOpen(true)}
              title="Preview & Fine-tune Trim"
              className="flex items-center justify-center gap-1 rounded-lg border border-white/[0.08] bg-white/[0.03] px-2.5 py-2 text-xs font-medium text-zinc-300 transition hover:bg-white/[0.08] hover:text-white active:scale-[0.98] cursor-pointer"
            >
